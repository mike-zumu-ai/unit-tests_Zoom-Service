...
namespace pipelines {
    ...
    inline pcm2mp3::pcm2mp3(const unsigned int rate, const unsigned int channels, const unsigned int depth, std::vector<uit8_t> format)
    ...
    inline void pcm2mp3::start(const std::function<void(const std::vector<uit8_t>&)>& callback) {
        ...
        processing_thread_ = std::thread([this, &callback]() {
            while (!processing_stop_) {
                ...
                if (gst_buffer_map(buffer, &map_info, GST_MAP_READ)) {
                    callback(std::vector<uit8_t>(reinterpret_cast<uint8_t*>(map_info.data), map_info.size));
